...
export default function ProductDetail({
  ...
  // Calculate slider images safely
  const sliderImages = project?.images?.slice(1) || []; // Only images from index 1 onwards
  ...
  useEffect(() => {
    if (project && project.images && project.images.length > 1) {
      setCurrentImageIndex(0); // Reset to first slider image (index 1 of original array)
    }
  }, [project]);
  ...
  return (
    <div className="relative">
      ...
      {/* Large Image Slider Section - Only show if there are slider images */}
                ...
                {sliderImages.length > 1 && (
                  <>
                    ...
                    <button
                      ...
                      className="hidden sm:flex absolute right-0 translate-x-full top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 xl:w-16 xl:h-16 items-center justify-center transition-all duration-200 hover:scale-110 text-black hover:text-gray-600 z-10"
              ...
              <div className="">
                <div className="flex gap-1 xs:gap-1 sm:gap-2 md:gap-3 lg:gap-4 xl:gap-5 2xl:gap-6 3xl:gap-7 4xl:gap-8 5xl:gap-10 overflow-x-auto overflow-y-hidden justify-start pb-2 scrollbar-hide">
                  {sliderImages.map((image, index) => (
                    <button
                      key={image.id}
                      onClick={() => setCurrentImageIndex(index)} // Fixed: just use index since we're already in sliderImages array
